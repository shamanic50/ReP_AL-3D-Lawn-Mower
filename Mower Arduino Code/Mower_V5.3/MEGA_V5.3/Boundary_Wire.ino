...
/* Perimieter Wire Collision Motion
  ************************************************************************************/
  ...
  Serial.print(F("Inside (1) or Outside (0) Fence:  "));
  ...
  if ( ((perimeter.isInside(0)) == 0) && (senderOff == 0)  &&  (Error == 0) && (Perimeter_Wire_Mode == 1) && (Mower_Docked == 0) && (Mower_Stopped == 0) )  {
    Serial.println(F("Outside the perimeter fence!!"));
    ...
    Serial.print(F("Outside Wire Count is : "));             // Prints the status to the serial monitor
    ...
    if (Outside_Wire_Count < Outside_Wire_Count_Max) {
      ...
      Serial.println(F("Turning at Perimeter Fence."));
...
void TestforBoundaryWire()  {
  ...
  if (Perimeter_Wire_Mode == 1) {                                               // Perimeter use is ON - Perimter_USE can be turned on or off in the setup.
    ...
    if ( (MAG_Now > -20 ) && (MAG_Now < 20 )  ) {
      Serial.print(F("Sender Turned OFF!! "));
      senderOff = 1;
      Error = 6;
      MowerMotionStatus = 1;
    }
    ...
    if (   (MAG_Now < -50 ) || (MAG_Now > 50 )  ) {
      Serial.print(F("Sender ON "));
      senderOff = 0;
    }

    Serial.print(F(" - Sender Status: "));
    Serial.print(senderOff);
    Serial.print(F("  = "));
    if (senderOff == 0) Serial.println(F("ON"));
    if (senderOff == 1) Serial.println(F("OFF"));
    Serial.print(F("Mag Reading = "));
