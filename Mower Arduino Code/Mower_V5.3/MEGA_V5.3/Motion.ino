...
void SpinBlades()
{
  ...
  if (Blades_ON == 0) {                                     // Blades are turn off in settings and will not spin!
    void StopSpinBlades();
    Serial.println(F("TEST MODE - BLADES STOPPED!"));
  }
}
...
void Motion_Compass_PWM_Steering() {
  delay(7);
...
void Calculate_Motion_Mower_Status() {
  ...
  if (MowerMotionStatus == 2)  {                       // Slow motion of Mower to detect objects
    ...
    SetPins_ToGoForwards();
    Motion_GoFullSpeed();
    Serial.print(F("First Time Around - Ping set at: "));
  ...
  if (MowerMotionStatus <= 5 && MowerMotionStatus > 2) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("Mowing..        ");
    Serial.print(F("Up close ping - Ping set at: "));
    Serial.println(maxdistancesonar);
    delay(1);
  }
  ...
  if (MowerMotionStatus > 5) {
    ...
    if (Compass_Heading_Locked == 1) {            // if the Mower is tracking using the compass steer here
      Motion_Compass_PWM_Steering();              // Removes the full speed function if the mower is trying to hold to the compass heading.
      lcd.setCursor(0, 1);
      lcd.print("C-Mowing..     ");
      Serial.println(F("Compass PWM used..."));
    }

    lcd.setCursor(13, 1);
    lcd.print(MowerMotionStatus);
    Serial.print(F("Looking way ahead at this speed - Ping set at: "));
    delay(1);
    Serial.println(maxdistancesonar);
    delay(1);
    ...
    if (MowerMotionStatus > 150) {                    // 150 the max length for my garden. Adjust accordingly
      Serial.println(F("Better turn just incase.."));
      StopSpinBlades();
