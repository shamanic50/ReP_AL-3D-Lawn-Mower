...
void PrintBoundaryWireStatus() {
  ...
  Serial.print(F("Inside Variable: "));
...
void FindWireTrack()  {
  ...
  for (int i = 0; i <= 1; i++) {
    ...
    if (inside == false) {                                              // If the mower is outside the wire then run the following code.
      ADCMan.run();
      UpdateWireSensor();
      Serial.println(F("Mower is situated outside the perimeter wire fence"));
      PrintBoundaryWireStatus();
...
void Check_if_Docked () {
  ...
  if ((AmpsTX >= 0.15) && (AmpsTX <=4) ){                     // if Amps are between this there is a charge detected.  Amps above 4 are discounted as a miscommunication
    delay(1000);
