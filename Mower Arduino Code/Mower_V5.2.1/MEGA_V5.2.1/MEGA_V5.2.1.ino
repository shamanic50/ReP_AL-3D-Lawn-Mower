...
/************************************************************************************
        ReP_AL Mower Project V5.2.1
     ==========================================

Before loading the code
-----------------------
1. Make sure that your LCD address is updated
2. Make sure that the setings for your mower are correct
...
LiquidCrystal_I2C lcd(0x3F, 2, 1, 0, 4, 5, 6, 7, 3, POSITIVE);      // Test Mower
 ...
 ***********************************************************************************************/
  ...
  ****************************************************************************************************/
  ...
  int Mower_Turn_Delay_Left       = 2000;                       // Turn time of the Mower after it reverses at the wire.
  int Mower_Turn_Delay_Right      = 2300;                       //     a higher number increases the angle of turn.
  int Mower_Reverse_Delay         = 2000;                       // Reverseing time when at the wire. 

  int Track_Wire_Zone_1_Cycles    = 2000;                       // Number of Itterations the PID function does before the mower exits the wire track
  int Track_Wire_Zone_2_Cycles    = 4000;                       //    Therefore how long the mower is tracking the wire can be set = distance tracked.
  ...
  bool Set_Time                   = 1;                          // Turn to 1 to set time on RTC (Set time in Time tab Set_Time_On_RTC)  After setting time turn to 0 and reload sketch.
...
void setup() {
  ...
  Serial1.begin(9600);
  Serial.println(F(""));
  Serial.println(F("          SETTING UP THE MOWER..... Please Wait ........"));
  Serial.println(F("************************************************************"));
  Serial.println(F(" "));
...
void loop() {
  ...
  ***********************************************************************/
  ...
  Serial.print(F("Mower Docked Status = "));
  ...
  Serial.print(F("Loops completed: "));                              // Keeping track of the program health in the serial monitor
  Serial.println(MowerMotionStatus);                              // In Setup - first loop Motion Mower is set to 2
  ...
  ************************************************************************************/
  ...
  //Check_Serial_Communications_Amps();
  delay(5);
  ...
  Serial.println(F("1 added to the MowerMotionStatus"));
  MowerMotionStatus = MowerMotionStatus + 1;                                                    //Adds 1 to the Loop and back to the Top
  Serial.print(F("Mower Motion Status is now : "));
  Serial.println(MowerMotionStatus);
