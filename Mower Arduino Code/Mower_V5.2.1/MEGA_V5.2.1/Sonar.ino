void Ping_Sonars()   {
  ...
  if (  (distance1 < maxdistancesonar) ||  (distance2 < maxdistancesonar)  ||  (distance3 < maxdistancesonar)  ) {
    ...
    Serial.println(F("**** Object Registered ******"));
   }
   else  {
    ObjectRegistered = 0;
  }
  ...
  Serial.print(F("Object Count is: "));
  ...
  if (ObjectRegistered >= 2) {
    ...
    if ( distance1 < maxdistancesonar ||  distance2 < maxdistancesonar  ||  distance3 < maxdistancesonar && Error == 0 ) {
      lcd.setCursor(0, 1);
      lcd.print("Object:          ");
      Serial.println(F("Object Detected by Sonar"));
      ...
      if ( distance1 < maxdistancesonar ||  distance2 < maxdistancesonar ) {
        ...
        lcd.setCursor(0, 8);
        lcd.print("<-- Go Left       ");
        SetPins_ToTurnLeft(); Serial.println(F(""));                                                  // Calls the motor function turn Left
        delay(200);
